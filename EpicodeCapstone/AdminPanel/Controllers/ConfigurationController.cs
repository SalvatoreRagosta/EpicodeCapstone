...
namespace AdminPanel.Controllers
{
    public class ConfigurationController : BaseController
    {
        ...
        [HttpGet]
        [RolesRightsAuthorizationHelper((int)EntitiesEnum.ScreensLocalization, 0, 0, 0, (short)UserRightsEnum.View_All, 0)]
        public async Task<IActionResult> GetEntitiesListByAppModuleId(int AppModuleId)
        {
            // ✅ Main Model
            ConfigurationModel model = new ConfigurationModel();
            ...
            try
            {
                ...
                model.EntityListSearchDropDown = await this._configurationServicesDAL.GetEntitiesListDAL(EntitySearchDrowdownFormData);
                model.EntityListSearchDropDown = model?.EntityListSearchDropDown?.Where(x => x.EntityTypeId == (short)EntityTypesEnum.Screen || x.EntityTypeId == (short)EntityTypesEnum.ReactComponent).ToList();

                var result = (from o in model?.EntityListSearchDropDown
                              select new
                              {
        ...
        public async Task<IActionResult> SaveScreenLocalizationLabel(LocalizationLabelsInfoEntity FormData, int DataOperationType = (short)DataOperationType.Insert)
        {
            ...
            try
            {
                ...
                if (FormData.AppModuleId == null || FormData.AppModuleId < 1)
                {
                    return Json(new { success = false, message = "Please select app module!" });
                }
                #endregion
                ...
                else
                {
                    ...
                    var entityDetail = await _configurationServicesDAL.GetEntityDetailByIdDAL(FormData?.EntityId ?? 0);

                    screenLocalizationData.AppModuleId = entityDetail?.AppModuleId ?? 0;
