    ...
    public class HomeController : BaseController
    {
        ...
        public async Task<IActionResult> GetDashboardChartsData(string FromDate, string ToDate)
        {
            ...
            try
            {
                ...
                model.dashboardLifeTimeStatistics = await this._homeServicesDAL.GetDashboardLifetimeStatisticsDAL(FromDate, ToDate, LoginUserId);
                if (LoginUserId > 0 && model.dashboardLifeTimeStatistics != null)//--Do not show total users if login user is vendor
                {
